 ...
 */			
@WebServlet("/backend/exambaseservlet.controller2")
public class ExamBaseServlet2 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (optA == null || optA.length() == 0) {
			errors.put("open","$(\"#dialog\").dialog(\"open\");");
			errors.put("optA", "請輸入A選項");
		}
		if (optB == null || optB.length() == 0) {
			errors.put("open","$(\"#dialog\").dialog(\"open\");");
			errors.put("optB", "請輸入B選項");
		}
		if (optC == null || optC.length() == 0) {
			errors.put("open","$(\"#dialog\").dialog(\"open\");");
			errors.put("optC", "請輸入C選項");
		}
		if (errors != null && !errors.isEmpty()) {
			errors.put("open","$(\"#dialog\").dialog(\"open\");");
			errors.put("wrong", "格式錯誤請，請重新輸入");
			request.getRequestDispatcher("/backend/activityInsert.jsp")
